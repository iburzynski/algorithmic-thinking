"""
Helper functions and constants for Application component of Module 3
...
def plot_clusters(data_table, cluster_list, method, num_counties, 
    ...
    if not draw_centers:
        for cluster_idx in range(len(cluster_list)):
            ...
            for fips_code in cluster.fips_codes():
                ...
                plt.scatter(x=[line[1]], y=[line[2]], s=circle_area(line[3]), 
                               lw=1, facecolors=cluster_color, 
    ...
    # Add cluster centers and lines from center to counties            
        ...
        for cluster_idx in range(len(cluster_list)):
            ...
            for fips_code in cluster.fips_codes():
                line = data_table[fips_to_line[fips_code]]
                plt.plot( [cluster_center[0], line[1]], [cluster_center[1], 
                           line[2]], cluster_color, lw=1, zorder=2)
        for cluster_idx in range(len(cluster_list)):
            cluster = cluster_list[cluster_idx]
            cluster_color = COLORS[cluster_idx % len(COLORS)]
            cluster_center = (cluster.horiz_center(), cluster.vert_center())
            ...
            plt.scatter(x=[cluster_center[0]], y=[cluster_center[1]], 
                           s=circle_area(cluster_pop), lw=2, facecolors="none", 
                           edgecolors="black", zorder=3)
    title = f"{method.capitalize()} Clustering Map for {num_counties} Counties"
    title += f" ({len(cluster_list)} clusters)"
